...
function score($dice)
{
    $valuesCount = array_count_values($dice);
    $points = 0;
    $tripletsPoints = [
        1 => 1000,
        6 => 600,
    ...
    foreach ($tripletsPoints as $value => $tripletScore) {
        if (isset($valuesCount[$value]) && $valuesCount[$value] >= 3) {
            $points += $tripletScore;
            $valuesCount[$value] -= 3;
        }
    }

    foreach ($singlePoints as $value => $singleScore) {
        if (isset($valuesCount[$value])) {
            $points += $valuesCount[$value] * $singleScore;
        }
    }
