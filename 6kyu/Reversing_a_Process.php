...
<?php
function decode(string $r): string {
    preg_match('/^(\d+)(\D+)$/', $r, $matches);
    ...
    foreach (str_split($encodedString) as $ch) {
        $x = ord($ch) - ord('a');
        $originalX = ($x * $modInv) % 26;
        $decodedString .= chr($originalX + ord('a'));
    }
...
See on CodeWars.com: https://www.codewars.com/kata/5dad6e5264e25a001918a1fc
