...
function balancedNum($num) {
    ...
    if ($length % 2 != 0) {
        $leftSum = array_sum(str_split(substr($str, 0, floor($mid))));
        $rightSum = array_sum(str_split(substr($str, ceil($mid))));
    } else {
        $leftSum = array_sum(str_split(substr($str, 0, $mid - 1)));
        $rightSum = array_sum(str_split(substr($str, $mid + 1)));
    }
